...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	// -------------------------------------------------------------------
	// 1. Basic Operation --> Create a non-empty myBinarySearchTree from a
	// myBinarySearchNode: create_from_binary_search_node()
	// -------------------------------------------------------------------
	// public myBinarySearchTree create_from_element_and_trees(myBinarySearchNode<T>
	// n); --> Java does not support constructors in interfaces
	...
	public myBinarySearchTreeImpl(myBinarySearchNode<T1, T2> node) throws myException {
		...
		// 2. However, there is a detail. Now we have to check that the tree we are
		// constructing is ordered as well.
		if (this.root != null) {
			...
			if (this.my_left_tree().my_is_empty() == false) {
				...
				if ((newNode.getKey()).compareTo(this.my_root().getKey()) >= 0)
					throw new myException(
							"The tree being constructed is not ordered, as the root is smaller than the elements of its left subtree");
			}
			...
			if (this.my_right_tree().my_is_empty() == false) {
				...
				if ((newNode.getKey()).compareTo(this.my_root().getKey()) <= 0)
					throw new myException(
							"The tree being constructed is not ordered, as the root is bigger than the elements of its right subtree");
			}
	...
	public myBinarySearchNode<T1, T2> my_find(T1 key) {
		...
		// 1.2. If it is not empty
			...
			// Otherwise, we have to keep looking
				...
				// Case B: If the element is smaller than the root, we keep looking in the left
				// subtree.
				if (k < 0)
					res = this.my_left_tree().my_find(key);
			// Case C: If the element is bigger than the root, we keep looking in the right
			// subtree.
				else
	...
	public myBinarySearchTree<T1, T2> my_insert(T1 key, T2 info) {
		...
		// 1.1 If the tree is empty, then we insert the element by creating a new tree
		// just containing this element
		if (this.my_is_empty() == true) {
		...
		// 1.2. If the tree is not empty
			...
			// Otherwise, we have to keep looking
				...
				// Case B: If the element is smaller than the root, we insert the node in the
				// left subtree.
				if (k < 0) {
					myBinarySearchTree<T1, T2> myTree1 = this.my_left_tree().my_insert(key, info);
					this.root.setLeft(myTree1.my_root());
				}
				// Case C: If the element is bigger than the root, we insert the node in the
				// right subtree.
				else {
	...
	public myBinarySearchTree<T1, T2> my_remove(T1 key) {
		...
		if (this.my_is_empty() == false) {
			...
			if (k == 0) {
				...
				// Case A1: If both subtrees are non-empty: We create a new tree with minimum of
				// right subtree as root. Also, we remove the minimum of right subtree from the
				// proper right subtree
				...
				// Case A2: If left subtree is empty, the resulting tree will be the right
				// subtree
				if ((b1 == true) && (b2 == false))
					this.root = this.my_right_tree().my_root();
				// Case A3: If right subtree is empty, the resulting tree will be the left
				// subtree
				if ((b1 == false) && (b2 == true))
			...
			// Otherwise, we have to keep removing it in the subtrees
				...
				// Case B: If the element is smaller than the root, we remove the node in the
				// left subtree.
				if (k < 0) {
					myBinarySearchTree<T1, T2> myTree1 = this.my_left_tree().my_remove(key);
					this.root.setLeft(myTree1.my_root());
				}
				// Case C: If the element is bigger than the root, we remove the node in the
				// right subtree.
				else {
	...
	// -------------------------------------------------------------------
	// 9. Additional Operation --> Check the max number of levels of
	// myBinarySearchTree: my_length
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// 10. Additional Operation --> Count the amount of nodes of myBinarySearchTree:
	// my_node_count
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// 11. Additional Operation --> Count the amount of leaf nodes of
	// myBinarySearchTree: my_leaf_count
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// 12. Traversal Operation --> Traverse myBinarySearchTree in in-order:
	// my_inorder
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// 13. Traversal Operation --> Traverse myBinarySearchTree in pre-order:
	// my_preorder
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// 14. Traversal Operation --> Traverse myBinarySearchTree in post-order:
	// my_postorder
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// 15. Additional Operation --> Get maximum element at myBinarySearchTree:
	// my_maximum
	// -------------------------------------------------------------------
	public myBinarySearchNode<T1, T2> my_maximum() throws myException {
		myBinarySearchNode<T1, T2> res;

		// 1.1 If the tree is empty, then there is no minimum, so an exception is to be
		// raised
		if (this.my_is_empty() == true)
		...
		// 1.2. If it is not empty
			...
			if (r.my_is_empty() == true)
				res = this.my_root();
			// 1.2.2. If the right tree is non-empty, we keep looking for the maximum in the
			// right subtree.
			else
				res = r.my_maximum();
		}
	...
	// -------------------------------------------------------------------
	// 16. Additional Operation --> Get minimum element at myBinarySearchTree:
	// my_minimum
	// -------------------------------------------------------------------
	public myBinarySearchNode<T1, T2> my_minimum() throws myException {
		...
		// 1.2. If it is not empty
			...
			if (l.my_is_empty() == true)
				res = this.my_root();
			// 1.2.2. If the left tree is non-empty, we keep looking for the minimum in the
			// left subtree.
			else
				res = l.my_minimum();
		}
	...
	// -------------------------------------------------------------------
	// 17. Assignment 2 - Operation 1 --> Count how many nodes are there at level k
	// of myBinarySearchTree: my_count_at_level
	// -------------------------------------------------------------------
	public int my_count_at_level(int level) {
		// 1. We create out output variable
		int count = 0;

		// 2. We do our reasoning

		// Case 1: If empty
		if (this.my_is_empty())
			count = 0;
		else {
			// Case 2: Non-empty and Level 1
			if (level == 1)
				count = 1;
			// Case 2: Non-empty and Level > 1
			else {
	...
	// -------------------------------------------------------------------
	// 18. Assignment 2 - Operation 2 --> Check if myBinarySearchTree is balanced:
	// my_is_balanced
	// -------------------------------------------------------------------
	public boolean my_is_balanced() {
		myBinarySearchTree<T1, T2> t1;
		myBinarySearchTree<T1, T2> t2;
		boolean res;
		...
		if (this.my_is_empty()) {
			...
			t1 = this.my_right_tree();
			t2 = this.my_left_tree();
			if (t1.my_length() == t2.my_length()) {
				res = true;
			} else
				res = false;
		}
	...
	// -------------------------------------------------------------------
	// 19. Assignment 2 - Operation 3 --> Count how many nodes are smaller in
	// myBinarySearchTree: my_count_smaller_nodes
	// -------------------------------------------------------------------
	public int my_count_smaller_nodes(T1 key) {
		...
		if (my_is_empty()) {
			...
			int comp = key.compareTo(this.my_root().getKey());

			if (comp == 0) {
				res = t2.my_node_count();
			} else if (comp < 0) {
				res = t2.my_count_smaller_nodes(key);
			} else if (comp > 0) {
				res = 1;
				res += t1.my_count_smaller_nodes(key);
				res += t2.my_node_count();
			}
		}
		return res;
	...
	// -------------------------------------------------------------------
	// 20. Assignment 2 - Operation 4 --> Return the level in myBinarySearchTree
	// were the node is located : my_find_node_at_level
	// -------------------------------------------------------------------
	public int my_find_node_at_level(T1 key) {
		...
		if (my_is_empty()) {
			...
			if (k == 0) {
				...
				res1=t1.my_find_node_at_level(key);
				if(res1==-1) {
					return res1;
				}
				else 
				...
				res1=t2.my_find_node_at_level(key);
				if(res1==-1) {
					return res1;
				}
				else 
